...
class FileLister:
    ...
    def __init__(self, path: str = "", data: dict = {}):
        """ This method initializes the FileLister class.
        ...
        Bu method FileLister sınıfını başlatır.
        ...
        os.chdir(self.path)
        self.path = os.getcwd()
        self.datas: dict = data  # { "name" : [depth(int) ,type (file/dir) , size , date , {childs}]  }
        self.data_str: str = ""
    ...
    Args:
        self: A reference to the `FileLister` object.
    ...
    Args:
        self: FileLister sınıfının bir örneği.
    ...
    Returns:
        ...
        if data is None: data = self.datas
        files = []
        for entry in os.listdir(path):
            entry_path = os.path.join(path, entry)
            if os.path.isdir(entry_path):
                data[entry] = [depth + 1, "dir", 0, self.unixToDate(os.path.getmtime(entry_path)), {}]
                maxNameLength = [max(FileLister.maxNameLength[0], len(entry)), max(FileLister.maxNameLength[1], depth)]
        ...
        for file in files:
            ...
            FileLister.maxNameLength[0] = max(FileLister.maxNameLength[0], len(file))
    ...
    Args:
        self: FileLister sınıfının nesnesi .
    ...
    """
        ...
        if data == {}: data = self.datas
        for key in data.keys():
            value = data[key]
            bas: str = ("|" + " " * FileLister.tab_size) * value[0]
            folderCount: str = (
                "(" + str(len(value[4])) + ")" if value[1] == "dir" else "") if FileLister.details_folderCount else ""
            size: str = str(round(value[2] / (1024 * 1024), 2)) + "mb " if FileLister.details_size else ""
            date: str = str(value[3]) if FileLister.details_date else ""
            arabosluk: str = " " * (width + maxNameLength[0] + maxNameLength[1] * tab_size - len(
                bas + key + folderCount + size + date)) if len(size + date) > 0 else ""
            self.printAll_dotted = not self.printAll_dotted
            if self.printAll_dotted: arabosluk = arabosluk.replace(" ", "-")
            self.data_str += f"{bas}{key}{folderCount}{arabosluk}{size}{date}\n"
            print(f'{bas}{key}{folderCount}{arabosluk}{size}{date}')
            if value[1] == "dir" and len(value[4]) > 0: self.printAll(value[4])
    
    def saveFileLister(self):
        ...
        The formatted output is stored in the `data_str` attribute of the `FileLister` object.

        Filename Generation:
            * A unique filename is created starting with "FileLister_".
        ...
        Dosya Adı Oluşturma:
            * Dosya adı "FileLister_" ile başlar.
        ...
        Returns:
            None
        """
        with open("FileLister_" + self.unixToDate(datetime.datetime.now().timestamp()).replace(":", ".") + ".txt",
                  "w") as f:
            f.write(self.data_str)
        print("File saved at:",os.getcwd()," and its starts width: FileLister_******.txt")
